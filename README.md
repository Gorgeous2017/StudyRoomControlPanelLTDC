...
其中主要的难点是被点击的标签与另外两个标签的交互。传进控制函数的是一个图标结构体指针，事先并不知道它的下标；并且我们并不知道事先哪一个位置的标签会被点击，这就导致了我们没法确定是使用当前下标的+1、+2、还是-1的方式访问另外两个标签。 各个标签的数组顺序是贴在一起的
